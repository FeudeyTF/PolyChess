	...
	internal class StudentCommands : TelegramCommandAggregator
	{
		...
		private async Task CreativeTaskApprove(TelegramButtonExecutionContext ctx)
		{
			DiscreteMessage discreteMessage = new(
				...
				[new TelegramMessageBuilder("Введите дополнительное сообщение. -, если не отправлять дополнительное сообщение")],
				...
				if (args.Responses.Length == 1 && ctx.Query.Message != null)
				{
					if (ctx.Query.Message != null)
					{
						...
						if (student != null)
						{
							...
							var additionalMessage = args.Responses[0].Text;

							if (!string.IsNullOrEmpty(additionalMessage) && additionalMessage != "-")
								await ctx.Provider.SendMessageAsync(new TelegramMessageBuilder($"Преподаватель оставил дополнительное сообщение: {additionalMessage}"), telegramId);
