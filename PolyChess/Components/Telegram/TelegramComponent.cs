...
namespace PolyChess.Components.Telegram
{
	internal class TelegramComponent : IComponent
	{
		...
		private async Task HandleCommandNotFound(TelegramCommandExecutionContext ctx)
		{
			await ctx.ReplyAsync($"Команда '{ctx.Message.Text}' не была найдена!");
		}

		private async Task HandleButtonNotFound(TelegramButtonExecutionContext ctx)
		{
		}
	}
}
