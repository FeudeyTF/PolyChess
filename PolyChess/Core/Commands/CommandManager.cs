namespace PolyChess.Core.Commands
{
	internal class CommandManager<TContext> where TContext : ICommandExecutionContext
	{
		...
		private readonly Func<TContext, Task> _onCommandNotFound;

		public CommandManager(List<ICommandAggregator<TContext>> aggregators, Func<TContext, Task> onCommandNotFound)
		{
		...
		public async Task ExecuteAsync(string name, TContext context)
		{
			...
			{
				await _onCommandNotFound(context);
			}
		}
	}
}
