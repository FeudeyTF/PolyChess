...
namespace PolyChess.Core.Telegram.Messages.Pagination.Builders
{
	...
	{
		private readonly Func<TValue, int, ChatId, string> _valueToString;

		public SimplePaginationMessageBuilder(Func<TValue, int, ChatId, string> valueToString)
		{
		...
		{
			var text = BuildMessageText(values, page, totalPages, chatId);
			var keyboard = BuildPaginationKeyboard(page, totalPages, type);
			return new TelegramMessageBuilder(text, replyMarkup: keyboard);
		}

		private string BuildMessageText(List<TValue> items, int page, int totalPages, ChatId chatId)
		{
			...
			for (int i = 0; i < items.Count; i++)
				text += $"{_valueToString(items[i], i, chatId)}\n";
