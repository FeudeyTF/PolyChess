...
namespace PolyChess.Core.Telegram.Messages.Pagination
{
	internal class PaginationMessage<TValue> : ITelegramMessage
	{
		...
		{
			var itemsList = ItemsRetriever(new Message() { Chat = new() { Id = chatId.Identifier ?? 0 } });
		...
		{
			IEnumerable<TValue> itemsList = ItemsRetriever(oldMessage);
			var totalItems = itemsList.Count();
			var totalPages = (int)Math.Ceiling((double)totalItems / ItemsPerPage);
		...
		private List<TValue> GetPageValues(IEnumerable<TValue> values, int page)
		{
