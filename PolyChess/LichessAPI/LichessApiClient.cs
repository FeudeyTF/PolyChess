...
namespace LichessAPI
{
    ...
    {
        public const string LichessApiUrl = "https://lichess.org/api/";
        ...
        {
            HttpRequestMessage msg = new(method, LichessApiUrl + string.Join("/", path));
        ...
        {
            var msg = await _httpClient.SendAsync(message);
            OnMessageSent(message, msg);
            return msg;
        }

        internal async Task<Stream> GetFileAsync(params object[] path)
            => await _httpClient.GetStreamAsync(LichessApiUrl + string.Join("/", path));

        internal async Task<string> GetFileStringAsync(params object[] path)
            => await new StreamReader(await _httpClient.GetStreamAsync(LichessApiUrl + string.Join("/", path))).ReadToEndAsync();
